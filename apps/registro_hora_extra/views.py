...
class ExportarParaCSV(View):
    def get(self, request):
        ...
        bancoHoras = RegistroHoraExtra.objects.filter(utilizada=False)

        writer = csv.writer(response)
        writer.writerow(['ID', 'Motivo', 'Funcionario', 'Rest, Funcionario', 'Horas'])

        for registro in bancoHoras:
            writer.writerow([registro.id, registro.motivo, registro.funcionario.nome,
